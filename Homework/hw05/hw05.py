...
def make_withdraw(balance, password):
    ...
    error_time = 0
    error_passowrd = []
    def withdraw(amount,password_new):
        nonlocal error_time,balance
        if password_new != password and error_time <3:
            error_passowrd.append(password_new)
            error_time += 1
            return 'Incorrect password'
        elif error_time ==3:
            return "Too many incorrect attempts. Attempts: ['%s', '%s', '%s']" %(error_passowrd[0],error_passowrd[1],error_passowrd[2])
        else:
            if balance < amount:
                return 'Insufficient funds'
...
def repeated(t, k):
    ...
    a = next(t)
    b = next(t)
    def helper(element_one,element_two,time):
        if element_one == element_two:
            time += 1 
            if time == k:
                return element_one
            else:
                return helper(element_two, next(t),time)
        else:
            return helper(element_two,next(t),1)   
    return helper(a,b,1)     
