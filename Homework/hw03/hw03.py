...
def missing_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(i,left,last,min):
        if left == 0:
            return 0
        if last ==0:
            return i+1 -min
        else:
            if str(last) in str(left):
                return helper(i,left,last-1,min = last)
            else:
                return helper(i+1,left,last-1,min)
    return helper(0,n//10,n%10-1,0)
def count_change(total):
    ...
    True
    """
    def find_max_2(total,pow_2):
        if total%pow_2 == total:
            return helper(total,pow_2/2) 
        else:
            return find_max_2(total,pow_2*2)
    def helper(total,max):
        if max == 1:
            return 1
        elif total == max:
            return 1+helper(total,max/2)
        elif total <max:
            return helper(total,max/2)
        else:
            return helper(total-max,max)+helper(total,max/2)
    return find_max_2(total,1)
