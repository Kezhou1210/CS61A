...
class VendingMachine:
    ...
    def vend(self):
        if self.inventory ==0 :
            ...
            if self.fund < self.price:
                return 'You must add $%d more funds.' %(self.price - self.fund)
    ...
    def add_funds(self,money):
        if self.inventory == 0:
            return 'Inventory empty. Restocking required. Here is your $%d.' %(money)
        else:
            self.fund +=money
            return 'Current balance: $%d' %(self.fund)
    def restock(self,num):
        self.inventory += num
        return 'Current %s stock: %d' %(self.name,self.inventory)
...
def is_bst(t):
    ...
    def bst_max(t,max):
        while t.is_leaf() == False:
            if len(t.branches)==1: 
                if t.branches[0].label < max:
                    t = t.branches[0]
                else:
                    return False
            else:
                if t.branches[1].label < max:
                    t= t.branches[1]
                else:
                    return False
        return True            
...
def store_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***" #let while loop start!
    num= []
    while n != 0:
        n,last = n//10, n%10
        num.append(last)
    def helper(link,num):
        if num == []:
            ...
            link.first,link.rest = num[0],Link(link.first,link.rest)
            return helper(link,num[1:])
    return helper(Link(num[0]),num[1:])
...
def path_yielder(t, value):
    ...
    for branch in t.branches:
        for val in path_yielder(branch,value):
            yield [t.label]+val
            "*** YOUR CODE HERE ***"
...
def remove_all(link , value):
    ...
    elif link != Link.empty and link.rest == Link.empty:
        if link.first == value:
            link = link.rest
